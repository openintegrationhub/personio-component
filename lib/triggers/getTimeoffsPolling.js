...
async function processTrigger(msg, cfg, snapshot = {}) {
  ...
  async function emitData() {
    ...
    const timeoffs = await getEntries(token, snapshot, 'time-offs', cfg);

    console.log(`Found ${timeoffs.result.length} new records.`);

    if (timeoffs.result.length > 0) {
      timeoffs.result.forEach((elem) => {
        const newElement = {};
      ...
      snapshot.lastUpdated = timeoffs.result[timeoffs.result.length - 1].lastUpdate;
