...
async function processAction(msg, cfg) {
  ...
  async function emitData() {
    ...
    const attendanceObject = msg.body.data;

    // if (recordUid && recordUid !== "" && recordUid !== "undefined") {
    // Conflict Management implementation
    // const cfmResponse = await resolve(msg, token, "events");

    // If option is set, insert reference for pipedrive ref management instead of using cfm
    // if (cfg.sourceApp) {
