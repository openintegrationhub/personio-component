...
async function deleteAttendance(msg, token) {
  try {
    ...
    const uri = `https://api.snazzycontacts.com/api/event/${uid}`;
    ...
    const event = await request.delete(options);
...
async function processAction(msg, cfg) {
  ...
  async function emitData() {
    const reply = await deleteAttendance(msg, token);
