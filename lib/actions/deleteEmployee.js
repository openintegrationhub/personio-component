...
async function deleteEmployee(msg, token) {
  try {
    ...
    const uri = `https://api.snazzycontacts.com/api/user/${uid}`;
    const options = {
      uri,
      json: true,
      headers: {
    ...
    const user = await request.delete(options);
    return user;
  } catch (e) {
    return e;
  }
...
async function processAction(msg, cfg) {
  ...
  async function emitData() {
    const reply = await deleteEmployee(msg, token);
