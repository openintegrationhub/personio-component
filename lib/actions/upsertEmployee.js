...
async function processAction(msg, cfg, snapshot = {}) {
  ...
  async function emitData() {
    ...
    if (
      ...
      && employeeObject.employee.email
      && employeeObject.employee.email.trim() !== ''
    ) {
      let reply;
      reply = await upsertObject(
        employeeObject,
        token,
        objectExists,
        'employees',
      ...
      if (!reply.body.success && isNewToken === false) {
        token = await getToken(cfg);
        reply = await upsertObject(
          employeeObject,
          token,
      ...
      console.log(`Entry ignored because no email supplied:${JSON.stringify(employeeObject)}`);
    }
