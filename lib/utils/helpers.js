  ...
  try {
    ...
    if (entries.body.data) {
      ...
      entries.body.data.forEach((employee) => {
        ...
        if (
          moment(employee.attributes.last_modified_at.value).isAfter(
 ...
 * @return {Object} - Array of person objects containing data and meta
 */
async function getEntries(token, snapshot, type) {
  // if (count) {
  //   uri = `${BASE_URI}/${type}?num=${count}`;
  // } else {
  //   uri = `${BASE_URI}/${type}`;
  // }
  // let token = getToken(cfg)
  const uri = `${BASE_URI}/company/${type}`;
  try {
...
async function getEntry(token, id, type) {
  const uri = `${BASE_URI}/company/${type}/${id}`;
  try {
    const requestOptions = {
      uri,
      json: true,
    ...
    console.log('these are the requestOptions:', requestOptions);
    const entry = await request.get(requestOptions);
